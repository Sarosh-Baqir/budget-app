import { mysqlTable, boolean, mysqlEnum, varchar, decimal, timestamp, int,} from "drizzle-orm/mysql-core";
...
export const transaction = mysqlTable("transaction", {
  ...
  labelId: int("label_id").notNull().references(() => label.id),
  ...
  status: mysqlEnum("status", ["pending", "completed", "cancelled", "none"]).default("none").notNull(),
